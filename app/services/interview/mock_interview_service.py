...
class MockInterviewService:
    ...
    async def start_session(
        ...
        Args:
            ...
            resume_context: Optional structured resume data for claim-based probing
...
"""
    ...
    async def _generate_session_questions(
        ...
        try:
            ...
            # When resume_context is provided, replace ~30-40% of questions
            # with resume-specific probing questions generated by the LLM.
            if resume_context and isinstance(resume_context, dict):
                try:
                    resume_questions = await self._generate_resume_questions(
                        resume_context=resume_context,
                        interview_type=interview_type,
                        difficulty=difficulty,
                        count=max(1, num_questions // 3),
                        api_key=api_key,
                    )
                    if resume_questions:
                        # Replace the tail of the question list with resume-based ones
                        replace_count = len(resume_questions)
                        questions = questions[:num_questions - replace_count] + resume_questions
                        logger.info(f"📄 Injected {replace_count} resume-based questions into mock interview")
                except Exception as e:
                    logger.warning(f"Resume question generation failed (non-fatal): {e}")
    ...
    async def _generate_resume_questions(
        ...
        """Generate resume-specific probing questions using the LLM."""

        projects = resume_context.get("projects", [])
        achievements = resume_context.get("achievements", [])
        skills = resume_context.get("skills", [])
        summary = resume_context.get("experience_summary", "")

        projects_block = ""
        if projects:
            proj_lines = []
            for p in projects[:5]:
                name = p.get("name", "Unnamed") if isinstance(p, dict) else "Unnamed"
                tech = p.get("tech", []) if isinstance(p, dict) else []
                claims = p.get("claims", []) if isinstance(p, dict) else []
                proj_lines.append(f"  - {name} [{', '.join(tech[:5])}]")
                for c in claims[:3]:
                    proj_lines.append(f"    • {c}")
...
2. Ask HOW they achieved metrics, WHAT tradeoffs they made, WHY they chose their approach.
3. For behavioral: "Tell me about the time you [specific resume claim]..."
4. For technical: "In your [project], you mentioned [claim] — walk me through the architecture..."
5. Return ONLY a JSON array of objects with "question_text", "topic", "expected_points" (list of strings).
        ...
        try:
            ...
            for idx, item in enumerate(data[:count]):
                if not isinstance(item, dict):
                    continue
                q_text = item.get("question_text") or item.get("text", "")
                if not q_text:
                    continue
                questions.append(InterviewQuestion(
                    question_id=f"resume_{idx}_{uuid.uuid4().hex[:8]}",
                    question_text=q_text,
                    interview_type=interview_type,
                    difficulty=difficulty,
                    topic=item.get("topic", "resume-based"),
                    expected_points=item.get("expected_points", []),
                ))
            return questions

        except Exception as e:
            logger.warning(f"Resume question generation failed: {e}")
            return []
