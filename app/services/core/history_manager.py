...
class HistoryManager:
    ...
    async def initialize(self):
        ...
        async with self._lock:
            ...
            try:
                ...
                self._loaded = True
                logger.info(f"💾 History synced for user {self.user_id}: {len(self._tabs)} tabs")
            except Exception as e:
    ...
    async def _restore_from_backup(self):
        ...
        try:
            ...
            # Save restored data to main file (already under lock)
    ...
    async def _save_to_disk_internal(self):
        ...
        try:
            ...
            # Update mtime after write to avoid immediate re-sync
            if self.history_file.exists():
    ...
    async def save_search(
        ...
        # Guardrail: never persist empty searches (usually indicates upstream failure)
        # unless explicitly allowed.
        if not allow_empty and not questions:
            raise ValueError("Refusing to save empty history tab (0 questions)")
