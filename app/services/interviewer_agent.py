...
class InterviewerAgent:
    ...
    def get_questions(
        ...
        """
        questions_data = self.QUESTION_BANK.get(difficulty, self.QUESTION_BANK[QuestionDifficulty.MEDIUM])
        
        # Take first 'count' questions (or cycle if needed)
        selected = questions_data[:count]
        ...
        logger.info(f"Generated {len(questions)} questions at {difficulty} difficulty")
        return questions
...
CONTENT_RELEVANCE: <assessment>"""
    ...
    def format_tts_text(self, question: PracticeInterviewQuestion) -> str:
        """
        Format question text for TTS with natural, conversational delivery.
        Makes the interviewer sound like asking questions with proper intonation.
        
        Args:
            question: Interview question
            
        Returns:
            Formatted text for natural TTS delivery with prosody hints
        """
        import random
        
        # Dynamic conversational intros - randomized for natural variety
        ...
        # Select intro based on question position
        total_questions = 5  # Default, can be made dynamic later
        
        if question.id == 1:
            intro = random.choice(first_question_intros)
        elif question.id >= total_questions:
            intro = random.choice(final_question_intros)
        else:
            intro = random.choice(transition_intros)
        
        # Process the question text to add natural pauses and emphasis
        question_text = self._add_natural_pauses(question.text)
        
        # Add intro with natural pause
        formatted = f"{intro} {question_text}"
        
        # Dynamic contextual prompts based on question type
        if question.category.lower() in ['behavioral', 'situational']:
            behavioral_prompts = [
                " ... Take a moment to think about a specific example.",
                " ... Think of a real situation from your experience.",
                " ... Consider a concrete example you can share.",
                " ... Recall a specific instance when this happened."
            ]
            formatted += random.choice(behavioral_prompts)
        
        elif question.category.lower() == 'technical':
            if question.difficulty == QuestionDifficulty.HARD:
                tech_prompts = [
                    " ... Take your time with this one.",
                    " ... This is a challenging question, so think it through.",
                    " ... No rush, think carefully about your answer."
                ]
                formatted += random.choice(tech_prompts)
        
        elif question.time_limit > 90:
            time_prompts = [
                " ... And you can take your time with this one.",
                " ... You have plenty of time for this.",
                " ... Take as much time as you need."
            ]
            formatted += random.choice(time_prompts)
