...
class PracticeModeService:
    ...
    def __init__(
        ...
        logger.info("Practice Mode Service initialized successfully")
    ...
    async def start_interview(
        ...
        try:
            ...
            if round_type == InterviewRound.FULL_INTERVIEW:
                ...
                breakdown = RoundConfigService.get_full_interview_breakdown(experience_years)
                
                logger.info(f"🎯 Full Interview Day breakdown: {breakdown}")
                
                # Generate questions for each round in sequence
                all_questions = []
                question_id_counter = 1
                
                for sub_round, sub_count in breakdown.items():
                ...
                logger.info(f"✅ Full interview generated: {len(questions)} total questions across {len(breakdown)} rounds")
            ...
            try:
                ...
                formatted_text = self.interviewer_agent.format_tts_text(first_question)
    ...
    async def get_next_question_after_acknowledgment(
        ...
        try:
            ...
            try:
                ...
                formatted_text = self.interviewer_agent.format_tts_text(next_question)
                result = await asyncio.wait_for(
                    self.tts_service.synthesize_async(formatted_text, audio_path),
                    timeout=15.0
                )
