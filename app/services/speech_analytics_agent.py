...
class SpeechAnalyticsAgent:
    ...
    def _calculate_confidence(self, audio: np.ndarray, sr: int) -> Tuple[float, float]:
        ...
        try:
            ...
            # Normalize variance to 0-1 scale (inverted)
            # Typical variance ranges from 500-5000 for normal speech
            # Values below 1000 = very confident (0.8-1.0)
            # Values 1000-3000 = moderate confidence (0.4-0.8)
            # Values above 5000 = low confidence (0.0-0.4)
            normalized_variance = min(pitch_variance / 5000, 1.0)
            confidence_score = max(0.0, 1.0 - normalized_variance)
            
            return confidence_score, pitch_variance
