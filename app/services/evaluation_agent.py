...
class EvaluationAgent:
    ...
    def __init__(self, api_key: str, model_name: str = "gemini-1.5-pro"):
        ...
        Args:
            api_key: Gemini API key (not directly used here, passed to LLM service)
            model_name: Model name (default: gemini-1.5-pro)
        """
        self.model_name = model_name
        logger.info(f"Evaluation Agent initialized (Universal Provider Support)")
    ...
    async def _call_llm(self, prompt: str, api_key: Optional[str] = None) -> str:
        ...
        try:
            text = await llm_service.generate_text(
                prompt=prompt,
                api_key=api_key,
                json_mode=True,  # Evaluation agent relies on JSON
                temperature=0.7,
                max_tokens=2048
            )
            
            if not text:
                raise ValueError("LLM response empty in evaluation")
                
            return text
            
        except Exception as e:
            logger.error(f"Error in LLM evaluation call: {e}")
            raise
