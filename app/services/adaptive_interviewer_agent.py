...
class AdaptiveInterviewerAgent:
    ...
    def __init__(self, gemini_api_key: str, gemini_model: str):
        ...
        logger.info(f"Adaptive Interviewer Agent initialized with {gemini_model}")
    ...
    async def evaluate_answer_comprehensively(
        ...
        try:
            ...
            prompt = f"""You are an expert technical interviewer evaluating a candidate's answer with precision and fairness.
...
- Confidence score: {metrics.confidence_score:.2f}/10

EVALUATE THE ANSWER COMPREHENSIVELY:

1. **CORRECTNESS SCORE** (0-100%):
   - 90-100%: Exceptional answer with all key points, excellent depth
   - 70-89%: Strong answer covering most points with good understanding
   - 50-69%: Adequate answer but missing key concepts or depth
   - 30-49%: Weak answer with significant gaps
   - 0-29%: Incorrect or irrelevant answer

2. **TECHNICAL ACCURACY**: Excellent/Good/Fair/Poor
   - Check for technical correctness, not just keywords
   - Verify understanding vs memorization

3. **KEY POINTS ANALYSIS**:
   - Which key concepts were mentioned correctly?
   - Which important points were missed?
   - Were there any misconceptions?

4. **STRENGTHS** (max 2):
   - What did they do well?
   - Strong points in their explanation

5. **IMPROVEMENT AREAS** (max 2):
   - What could be better?
   - Gaps in knowledge or explanation

6. **ACTIONABLE SUGGESTIONS** (max 2):
   - Specific, practical tips to improve
...
{{
  ...
  "detailed_feedback": "Brief overall assessment in 1-2 sentences"
}}

Be fair, precise, and helpful. Focus on CONTENT CORRECTNESS, not speech delivery."""

            # Use JSON mode for guaranteed valid output
            json_config = {
                "temperature": 0.2,
            ...
            try:
                ...
                if not response.text:
                    ...
                    if hasattr(response, 'prompt_feedback'):
                        logger.error(f"❌ Gemini evaluation blocked - Prompt feedback: {response.prompt_feedback}")
                    ...
                    logger.warning("⚠️ Gemini response blocked in evaluation (no text), using fallback")
                    return self._fallback_evaluation(transcript, key_points, metrics)
                    
            except Exception as gemini_error:
                # Catch "response.text requires valid Part" error
                logger.error(f"❌ Gemini API error during evaluation: {gemini_error}")
                logger.warning("⚠️ Using fallback evaluation due to API error")
                return self._fallback_evaluation(transcript, key_points, metrics)
            
            # Parse JSON response - should work directly with JSON mode
            import json
            import re
            ...
            try:
                ...
                logger.debug("✅ Evaluation JSON parsed successfully on first attempt")
            except json.JSONDecodeError as e:
                # Log the problematic JSON for debugging
                logger.warning(f"JSON mode failed in evaluation? Attempting extraction: {e}")
                logger.debug(f"Problematic JSON (first 500 chars): {text[:500]}")
                ...
                try:
                    evaluation = json.loads(text)
                    logger.debug("✅ Evaluation JSON repaired successfully")
                except json.JSONDecodeError as second_error:
                    logger.warning(f"Basic repair failed, trying aggressive repair")
                    text = self._aggressive_json_repair(text)
                    try:
                        evaluation = json.loads(text)
                        logger.debug("✅ Evaluation JSON aggressively repaired")
                    except json.JSONDecodeError as third_error:
                        logger.error(f"Answer evaluation failed: {third_error}")
                        logger.error(f"Failed JSON (first 300 chars): {text[:300]}")
                        # Use fallback evaluation
                        logger.warning("⚠️ All JSON parsing failed, using fallback evaluation")
                        return self._fallback_evaluation(transcript, key_points, metrics)
...
Generate exactly {count} questions with proper formatting INCLUDING key_points, expected_answer_template, AND question_type."""
    ...
    async def _call_gemini(self, prompt: str, api_key: Optional[str] = None) -> str:
        """Call Gemini API with JSON mode for guaranteed valid output."""
        def _invoke():
            if api_key:
                genai.configure(api_key=api_key)

            # Use JSON mode for structured output (prevents malformed JSON)
            json_config = {
                "temperature": 0.8,
                "top_p": 0.95,
                "top_k": 40,
            ...
            if not response.text:
                # Log detailed error info
                if hasattr(response, 'prompt_feedback'):
                    logger.error(f"❌ Gemini prompt blocked: {response.prompt_feedback}")
                if hasattr(response, 'candidates') and response.candidates:
                    candidate = response.candidates[0]
                    logger.error(f"❌ Finish reason: {candidate.finish_reason}")
                    if hasattr(candidate, 'safety_ratings'):
                ...
                raise ValueError("Gemini response blocked by safety filters (finish_reason=2). This is likely a false positive for interview content.")
...
- Mentorship: Growing and leading teams""",
    ...
    async def _generate_generic_questions(
        ...
        templates = generic_bank.get(difficulty, generic_bank[QuestionDifficulty.MEDIUM])
