...
class LocalTTSService:
    ...
    def _init_pyttsx3(self) -> bool:
        ...
        try:
            ...
            self.engine.setProperty('rate', 155)  # Slightly faster for natural conversation (150-160 WPM)
            self.engine.setProperty('volume', 0.95)  # Clear but not overwhelming
            
            # Try to set a specific voice if available
            voices = self.engine.getProperty('voices')
            if voices:
                # Prefer MALE voices for professional interviewer tone
                # Priority: Male voices (David, Mark) > Female voices (Zira, Hazel)
                preferred_voices = [
                    'david',     # Microsoft David (male, US English)
                    'mark',      # macOS male voice (Alex is also good)
                    'alex',      # macOS Alex (male, high quality)
                    'james',     # Some systems have James
                    'george',    # British male voice
                    'zira',      # Microsoft Zira (female, US English) - fallback
                    'hazel',     # macOS female voice - fallback
                ]
                ...
                for pref in preferred_voices:
                    for voice in voices:
                        if pref in voice.name.lower():
                            ...
                            logger.info(f"Selected TTS voice: {voice.name} for natural interviewer tone")
                            break
                    if selected_voice:
                        break
                
                if not selected_voice and voices:
                    # Fallback: try to find any male voice
                    for voice in voices:
                        if 'male' in voice.name.lower() or 'man' in voice.name.lower():
                            selected_voice = voice
                            self.engine.setProperty('voice', voice.id)
                            logger.info(f"Selected male voice: {voice.name}")
                            break
